...
public class Run implements ActionListener {
	...
	// 棋盘，不管
	public Run(JPanel jPanel, ArrayList<Point> list) {
		jp = jPanel;
		points = list;
		for (int i = 0; i < jp.getComponentCount(); i++) {
			Component comp = jp.getComponent(i);
			if (comp instanceof JButton) {
				JButton btns = (JButton) comp;
				btns.addActionListener(new Run());
				points.add(btns.getLocation());
			}
		}
	}
	...
	class ButtonListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			for (Point point : points) {
				Component comp = jp.getComponentAt(point);
				if (comp instanceof JButton) {
